...
const ProjectCard = ({ project }) => (
  <div className="flex flex-col lg:flex-row bg-white border border-gray-300 pt-8 rounded-lg overflow-hidden w-full shadow-lg transition-all duration-300 hover:bg-gray-100 mb-8 group">
    <div className="w-full lg:w-3/5 px-4 sm:px-6 lg:px-8 pb-4">
      <div className="flex flex-col mb-4">
        <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between">
          <h3 className="text-xl sm:text-2xl font-bold text-black">{project.title}</h3>
          <span className="text-gray-600 text-xs italic mt-1 sm:mt-0 sm:ml-2">{project.date}</span>
        </div>
      </div>
      <div className="flex flex-wrap items-center mb-4 space-x-1 sm:space-x-2">
        <span className="text-sm text-gray-600">Made with:</span>
        {project.techStack.map((tech, index) => (
          <Icon key={index} icon={tech.icon} className="md:w-8 md:h-8 h-6 w-6" />
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
        {project.demoLink !== 'N/A' && (
          <a target="_blank" href={project.demoLink} className="bg-gray-900 text-white py-2 px-4 rounded-full flex items-center justify-center space-x-2 hover:bg-gray-700 transition-colors">
        ...
        <a href={project.githubLink} target="_blank" className="border border-gray-900 text-gray-900 py-2 px-4 rounded-full flex items-center justify-center space-x-2 hover:bg-gray-100 transition-colors">
